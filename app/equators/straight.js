const { getFiveHighestCardsFromStraight, sortPokerHand } = require('./helpers/index');

module.exports = ((hand) => {
  if (!hand.combo){ return { highestHand: null, pokerHand: null } };
  let pokerHand = [];
  let allPossibleStraightCards = [];
  let isAceHighStraight = [1, 10, 11, 12, 13].every(val => hand.cardNumbers.includes(val));
  ...
  if (isAceHighStraight){
    ...
    // normal straight
    for (let startCard of hand.cardNumbers){
      if (startCard + 4 > 13){ 
      ...
      let isAStraight = (straightSequence.every(val => hand.cardNumbers.includes(val)) && startCard !== 1)
      if (isAStraight){
